...
import { LatinLexicon, LexiconEntry } from './lexicon.js';
import { VulgateData, BibleData, VulgateBook, Chapter } from './vulgate-data.js';
...
class VulgateReader {
    ...
    tooltip: HTMLElement;
    versesContainer: HTMLElement;
    welcomeMessage: HTMLElement;
    bookSelect: HTMLSelectElement;
    chapterSelect: HTMLSelectElement;
    prevButton: HTMLButtonElement;
    nextButton: HTMLButtonElement;
    pinnedBubble: HTMLElement | null = null;

    constructor() {
        this.tooltip = document.getElementById('tooltip') as HTMLElement;
        this.versesContainer = document.getElementById('verses-container') as HTMLElement;
        this.welcomeMessage = document.getElementById('welcome-message') as HTMLElement;
        this.bookSelect = document.getElementById('book-select') as HTMLSelectElement;
        this.chapterSelect = document.getElementById('chapter-select') as HTMLSelectElement;
        this.prevButton = document.getElementById('prev-chapter') as HTMLButtonElement;
        this.nextButton = document.getElementById('next-chapter') as HTMLButtonElement;
    ...
    loadPosition(bookKey: string, chapterNum: number) {
    ...
    populateBookSelect() {
        ...
        const otBooks: BookOption[] = [];
        const ntBooks: BookOption[] = [];
    ...
    populateChapterSelect(bookKey: string) {
    ...
    bindEvents() {
        ...
        this.bookSelect.addEventListener('change', (e) => {
            const bookKey = (e.target as HTMLSelectElement).value;
            this.currentBook = bookKey;
        ...
        this.chapterSelect.addEventListener('change', (e) => {
            const chapter = parseInt((e.target as HTMLSelectElement).value);
            if (!isNaN(chapter)) {
        ...
        document.addEventListener('click', (e) => {
            ...
            const bubble = target.closest('.word-bubble:not(.punctuation)') as HTMLElement | null;
    ...
    loadChapter(bookKey: string, chapterNum: number) {
    ...
    renderVerse(verseNum: string, verseData: string | { latin: string, english?: string }) {
    ...
    toggleTooltip(bubble: HTMLElement, event: MouseEvent) {
        if (this.pinnedBubble === bubble) {
            this.pinnedBubble = null;
            this.hideTooltip();
        } else {
    ...
    parseWords(text: string): WordInfo[] {
    ...
    createWordBubble(wordInfo: WordInfo): HTMLElement {
    ...
    showTooltip(bubble: HTMLElement, event: MouseEvent) {
        ...
        if (entry) {
            ...
            const lemmaEl = this.tooltip.querySelector('.tooltip-lemma') as HTMLElement;
            const lemmaSection = lemmaEl.closest('.tooltip-section') as HTMLElement;
            ...
            const meaningsList = this.tooltip.querySelector('.tooltip-meanings') as HTMLElement;
            ...
            const grammarSection = this.tooltip.querySelector('.tooltip-grammar') as HTMLElement;
            const grammarText = this.tooltip.querySelector('.tooltip-grammar-text') as HTMLElement;
            if (entry.grammar) {
            ...
            const posEl = this.tooltip.querySelector('.tooltip-pos');
            if (posEl) posEl.textContent = 'unknown';
    ...
    positionTooltipNearElement(element: HTMLElement) {
        const padding = 10;
...
class SearchController {
    reader: VulgateReader;
    overlay: HTMLElement;
    input: HTMLInputElement;
    resultsContainer: HTMLElement;
    isOpen: boolean = false;

    constructor(reader: VulgateReader) {
        this.reader = reader;
    ...
    performSearch() {
        ...
        if (englishMatches.length > 0) {
            const latinWords = [...new Set(englishMatches.slice(0, 3).map(r => r.latinWord as string))];
    ...
    checkNavigation(query: string): SearchResult | null {
        ...
        const bookKey = Object.keys(VulgateData).find(key => {
            ...
            return book.name.toLowerCase().startsWith(bookName) ||
                (book as any).abbrev?.toLowerCase() === bookName ||
    ...
    searchLexicon(query: string): SearchResult[] {
        const results: SearchResult[] = [];
        ...
        Object.keys(VulgateData).forEach(bookKey => {
            ...
            Object.keys(book.chapters).forEach(chapterNum => {
                ...
                Object.keys(chapter).forEach(verseNum => {
                    const verseData = chapter[verseNum] as string | { latin: string, english?: string };
                    const latinText = typeof verseData === 'object' ? verseData.latin : verseData;
                    const englishText = (typeof verseData === 'object' && verseData.english) ? verseData.english : '';

                    let matchType: 'latin' | 'english' | null = null;
                    let matchSnippet = '';
                                ...
                                setTimeout(() => {
                                    ...
                                    for (let el of Array.from(verseEls)) {
                                        if (el.textContent === verseNum) {
                                            ...
                                            (el.parentElement as HTMLElement).style.backgroundColor = 'rgba(212, 163, 115, 0.1)';
                                            setTimeout(() => (el.parentElement as HTMLElement).style.backgroundColor = '', 2000);
                                            break;
                                        }
                                    }
                                }, 100);
